      ...
      <Link href={`/products/${product.slug}`} className="block">
        ...
        <div className="relative aspect-[4/5] w-full overflow-hidden bg-[#F0EFED] flex items-center justify-center">
          ...
          <button
            ...
            className={`absolute top-2.5 right-2.5 sm:top-3 sm:right-3 z-10 p-2 rounded-full backdrop-blur-md transition-all shadow-xs ${
              saved
                ? "bg-rose-50/95 text-rose-600 border border-rose-200 scale-110"
          ...
          <div className="absolute bottom-2.5 left-2.5 sm:bottom-3 sm:left-3 z-10">
            <Badge
              ...
              className="text-[9px] sm:text-[10px] px-2 py-0.5 sm:px-2.5 sm:py-1 shadow-xs backdrop-blur-md bg-white/95 border border-zinc-200/60 font-semibold"
            >
        ...
        <div className="p-3.5 sm:p-4.5 space-y-2 sm:space-y-2.5">
          ...
          <div className="flex items-center gap-1.5 text-[10px] sm:text-[11px] font-bold text-burgundy uppercase tracking-wider">
          ...
          <div className="flex items-center justify-between text-xs text-zinc-600 gap-2">
            <span className="truncate text-[11px] sm:text-xs font-medium text-zinc-600">
              {product.material || "Fine Fabric"}
            </span>
            {product.style_code && (
              <span className="font-mono text-[9px] sm:text-[10px] text-zinc-600 bg-zinc-100 px-1.5 py-0.5 rounded flex-shrink-0 border border-zinc-200/80">
                {product.style_code}
              </span>
            )}
          </div>
          ...
          {(product.available_sizes || []).length > 0 && (
            <div className="pt-0.5 flex items-center gap-1 flex-wrap">
              ...
              {product.available_sizes?.slice(0, 4).map((size) => (
                <span
                  key={size}
                  className="px-1.5 py-0.5 rounded text-[9px] sm:text-[10px] font-semibold bg-zinc-100/80 text-zinc-700 border border-zinc-200/80"
                >
                  {size}
                </span>
              ))}
